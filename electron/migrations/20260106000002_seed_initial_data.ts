...
module.exports.up = async function ({ db }: any) {
  ...
  // Seed RBAC roles
  db.exec(
    `INSERT OR IGNORE INTO roles (code, name, description) VALUES ('SUPERADMIN', 'Super Administrator', 'Full system access')`
  );
  db.exec(
    `INSERT OR IGNORE INTO roles (code, name, description) VALUES ('CHECKER', 'Checker', 'Transaction input and verification')`
  );
  db.exec(
    `INSERT OR IGNORE INTO roles (code, name, description) VALUES ('LOADER', 'Operator Loader', 'Loader operation only')`
  );
  ...
  const insertUser = db.prepare(
    `INSERT OR IGNORE INTO users (id, username, email, password_hash, first_name, last_name, status) VALUES (?, ?, ?, ?, ?, ?, ?)`
  );
  ...
  db.exec(
    `INSERT OR IGNORE INTO user_roles (user_id, role_id) SELECT 1, id FROM roles WHERE code = 'SUPERADMIN'`
  );
  db.exec(
    `INSERT OR IGNORE INTO user_roles (user_id, role_id) SELECT 2, id FROM roles WHERE code = 'CHECKER'`
  );
  db.exec(
    `INSERT OR IGNORE INTO user_roles (user_id, role_id) SELECT 3, id FROM roles WHERE code = 'LOADER'`
  );
  db.exec(
    `INSERT OR IGNORE INTO user_roles (user_id, role_id) SELECT 7, id FROM roles WHERE code = 'CHECKER'`
  );
  db.exec(
    `INSERT OR IGNORE INTO user_roles (user_id, role_id) SELECT 8, id FROM roles WHERE code = 'LOADER'`
  );
