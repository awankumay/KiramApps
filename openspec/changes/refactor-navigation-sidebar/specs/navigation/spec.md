# Spec: RBAC-Integrated Sidebar Navigation
...
### Requirement: Navigation Component Structure

The application MUST use a single navigation component instead of multiple inconsistent implementations.

#### Scenario: Single navigation component

Given the application is running
When any authenticated page is rendered
Then the AppSidebarLayout component MUST wrap the page content
And the AppSidebarRBAC component MUST be used for navigation
And deprecated components (AppSidebar, AppNavigation) MUST NOT be imported

#### Scenario: No duplicate navigation

Given the user navigates between pages
When moving from Dashboard to Superadmin pages
Then only ONE sidebar MUST be visible at all times
And there MUST NOT be duplicate navigation bars or sidebars

## REMOVED Requirements

### Requirement: Top Bar Navigation (DEPRECATED)

The application previously used a top bar navigation (AppNavigation.tsx) which is now replaced by sidebar.

#### Scenario: Top bar is removed

Given the application is updated
When the app renders
Then the AppNavigation component MUST NOT be imported
And the top navigation bar MUST NOT be displayed
And all navigation MUST be handled by the sidebar

### Requirement: Hardcoded Dashboard Sidebar (DEPRECATED)

The Dashboard feature previously had its own sidebar with hardcoded data (AppSidebar.tsx) which is now replaced.

#### Scenario: Hardcoded sidebar is removed

Given the application is updated
When the Dashboard page renders
Then the AppSidebar component from Dashboard/Components MUST NOT be used
And navigation data MUST come from RouteConfig
And menu items MUST be filtered by permissions
