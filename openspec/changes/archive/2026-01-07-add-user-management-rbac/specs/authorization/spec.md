...
This capability provides role-based access control (RBAC) enforcement for the Surat Masuk Digital application. It enables permission checking at both backend (main process) and frontend (renderer) layers, ensuring users can only access features allowed by their assigned roles. The system supports offline-first operation with locally cached permissions.
