# Spec: Items Management

## Overview

Fitur manajemen Items memungkinkan pengguna untuk mengelola daftar item yang ditransaksikan dalam sistem, termasuk tracking riwayat perubahan harga.
...
- `MANAGE_ITEMS` - Permission untuk mengakses fitur ini
- `items:*` - IPC handlers untuk komunikasi main-renderer
- `ItemData`, `CreateItemData`, `UpdateItemData` - TypeScript types
- `PriceHistoryData` - Type untuk riwayat harga

## Implementation Notes

- Gunakan pattern yang sama dengan UsersPage untuk konsistensi
- Pastikan semua operasi database menggunakan prepared statements
- Validasi form dilakukan di sisi renderer sebelum mengirim ke backend
- Price history hanya dicatat saat harga berubah, bukan field lain
